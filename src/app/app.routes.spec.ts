...
describe('app.routes', () => {
  ...
  comPermissaoNomes().forEach((caminho) => {
    it(`/${caminho} tem canActivate proprio, senao a permissao nao e lida`, () => {
      ...
      expect(rota)
        .withContext(`rota /${caminho} nao encontrada`)
        .toBeTruthy();

      expect(rota.canActivate)
        .withContext(
          `/${caminho} declara data.permission mas nao tem canActivate. ` +
            `O guard no pai recebe o snapshot do pai, cujo data nao tem permission — ` +
            `a checagem nao acontece.`
        )
        .toContain(authGuard);
    });
  });
  ...
  it('toda permissao declarada nas rotas existe na whitelist do dominio', () => {
    ...
    declaradas.forEach((p) => {
      expect(PERMISSOES_WL as readonly string[])
        .withContext(`permissao '${p}' nao existe em WlPermissoesValidas`)
        .toContain(p);
    });
  });
  ...
  it('rotas publicas ficam fora da area protegida', () => {
    ...
    ['login', 'acesso-negado'].forEach((caminho) => {
      ...
      expect(rota).withContext(`/${caminho} deveria ser rota de topo`).toBeTruthy();
